...
- Minor fixes:
  - s/Marsmello/Marsmello/ in the Coin Control example
...
- Internal miner added back to Marsmello.  setgenerate now works, although it is generally a bad idea as it is significantly slower than external CPU miners.
...
Fix out-of-bounds check (Marsmello currently does not use this codepath, but we apply this
patch just to match Marsmello 0.8.5.)

0.8.4.1 changes
===============

CVE-2013-5700 Bloom: filter crash issue - Marsmello 0.8.3.7 disabled bloom by default so was 
...
nodes running with the -debug flag to crash, although it was lot less likely on Marsmello 
