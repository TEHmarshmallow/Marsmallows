...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Marsmello: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Marsmello Signed Message:\n";
...
{
    // Marsmello: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 21600); // Marsmello: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Marsmello: 1 day
static const int64 nTargetSpacing = 2 * 60; // Marsmello: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Marsmello: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("Marsmello-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Marsmello: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Marsmello: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Na also, hier hat der Frosch die Locken";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Marsmello: increase each by adding 2 to Marsmello's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarsmelloMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarsmelloMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MarsmelloMiner(CWallet *pwallet)
{
    printf("MarsmelloMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Marsmello-miner");
    ...
    try { loop {
        ...
        printf("Running MarsmelloMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MarsmelloMiner terminated\n");
        throw;
    }
}

void GenerateMarsmellos(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MarsmelloMiner, pwallet));
}
